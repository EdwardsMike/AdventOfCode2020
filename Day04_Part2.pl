...
# Handle EOF (there's probably a better way to do this)
#print "New Entry";
foreach ( @this_entry ) {
	my ( $key, $val ) = split /:/, $_;
	#print $key, $val;
...
foreach my $ent ( @entries ) {
	#foreach my $key ( sort keys %{ $ent } ) {
		#print $key, $ent->{ $key };
		#}
	#print '-------------------------';
	my @keys	= sort keys %{ $ent };
	# Fewer than 7 keys is definitely invalid, so don't 
	# bother examining values
	if ( @keys < 7 ) {
		next;
	}
	elsif ( @keys == 7 &&  grep { /cid/ } @keys ) {
		next;
	}
	else {
		if ( 
			( $ent->{ byr } >= 1920 && $ent->{ byr } <= 2002 ) &&
			( $ent->{ iyr } >= 2010 && $ent->{ iyr } <= 2020 ) &&
			( $ent->{ eyr } >= 2020 && $ent->{ eyr } <= 2030 ) &&
			( check_hgt( $ent->{ hgt } ) ) &&
			( $ent->{ hcl } =~ m/^#[0-9a-f]{6}$/ ) &&
			( grep { /$ent->{ ecl }/ } qw( amb blu brn gry grn hzl oth ) ) &&
			( $ent->{ pid } =~ m/^\d{9}$/ ) 
		) {
			$valid++;
		}
